...
export interface EmergencyBannerProps {
  ...
  visible?: boolean;
  /** Visual severity: "emergency" (red), "warning" (orange), "info" (cyan) */
...
const severityConfig = {
  emergency: {
    ...
    borderColor: "#FF0000",
    stripeClass: "hazard-stripes-black",
  },
  warning: {
    bg: "bg-eva-orange",
    text: "text-eva-black",
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          {/* Top hazard stripe */}
          <div className={`relative h-4 ${config.stripeClass}`} />
          ...
          <div className="relative px-6 py-8 flex flex-col items-center justify-center min-h-[120px]">
            ...
            <motion.h1
              animate={{ opacity: [1, 0.3, 1, 0.5, 1, 1, 0.2, 1] }}
            ...
            {subtext && (
              <motion.p
                ...
                className="mt-3 text-lg md:text-xl uppercase tracking-[0.3em] text-black/80 font-mono"
              >
                {subtext}
              </motion.p>
            )}
