...
import {
  AccordionBasicDemo,
  AccordionMultiDemo,
} from "@/components/docs/AccordionDemo";
...
```tsx
import { Accordion, AccordionItem } from "@mdrbx/nerv-ui";
```

## Basic Usage

<Callout type="info" title="INTERACTIVE">
  Click each panel header to expand/collapse. In single mode (default), opening
  one panel closes the others.
</Callout>
...
<ComponentPreview
  ...
  <AccordionItem id="status" title="SYSTEM STATUS">
    <div className="text-xs text-nerv-green font-mono">
